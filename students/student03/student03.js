...
if(vPulseIntensity>0.){vec3 pc=mix(vec3(1.),uPulseColors[0],.4);fc=mix(fc,pc,vPulseIntensity*.8);fc*=(1.+vPulseIntensity*1.2);gs*=(1.+vPulseIntensity);}
...
if(vPI2>0.){vec3 pc=mix(vec3(1.),uPulseColors[0],.3);fc=mix(fc,pc*1.2,vPI2*.7);fi+=vPI2*.8;}
...
renderer.domElement.addEventListener('click', e => { if (!cfg.paused) pulse(e.clientX, e.clientY); });
renderer.domElement.addEventListener('touchstart', e => { e.preventDefault(); if (e.touches.length && !cfg.paused) pulse(e.touches[0].clientX, e.touches[0].clientY); }, { passive: false });
